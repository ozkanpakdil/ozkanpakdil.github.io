...
Green build, slow pages. That combination has annoyed me for years: Playwright happily reports 136/136 passing while the browser quietly burns 90% of one core on a badly-behaved tab, or the whole runner sits at 99% memory and everything gets *suspiciously* slow. Test outcomes say nothing about **how much resource the tests consumed** — and resource waste in CI is exactly the kind of thing you want to fail loudly, not discover from user complaints later.
...
- **Any threshold breach with `fail-on-breach: true`** (the default) → the action fails naming each breached layer with peaks vs thresholds. With `false`, it only warns and stays green — handy for the first weeks on a new project while you calibrate.
...
- Parallel CDP mode shares one debug port — on Linux just use the `/proc` scanner.
