---
title: "MariaDB vs MySQL on a Real Application: an mpazari Benchmark"
date: 2026-09-23
description: "A controlled MariaDB 10.3 and MySQL 8.0 comparison using the real mpazari.com application and production-sized data."
tags: ["mysql", "mariadb", "performance", "benchmark", "databases", "k6"]
categories: ["Databases", "Performance"]
cover:
    image: cover.webp
    alt: MariaDB versus MySQL benchmark
---

I recently compared MariaDB and MySQL using the real [mpazari.com](https://www.mpazari.com)
application. The goal was not to produce a synthetic database benchmark. I wanted to answer a
more practical question: **does MariaDB really have an advantage when an application sends many
simple queries?**
...
data into MariaDB and MySQL and ran the same application against both.
...
- The same pre-cutover dump was restored into both engines.
- The same application build and runtime settings were used for both runs.
- The application was freshly restarted before each main run.
- Both databases listened only on `127.0.0.1`.
...
| Metric | MariaDB 10.3.39 | MySQL 8.0.42 |
|---|---:|---:|
| Average | **125.52 ms** | 132.40 ms |
| p(95) | **129.73 ms** | 139.43 ms |
| p(99) | 182.13 ms | **150.07 ms** |
| Maximum | **229.31 ms** | 243.90 ms |
| Requests | **16,025** | 15,930 |
| Failed requests | 0% | 0% |
| Database RSS peak | **141 MB** | 411 MB |
| Load average, 1-minute average / peak | **0.88 / 1.34** | 1.27 / 2.01 |

MariaDB was about 5% faster on average and about 7% faster at p(95). That is a real difference,
but not a dramatic one. Both engines had plenty of headroom and passed the latency thresholds
comfortably.

The more interesting result is the tail: MySQL's p(99) was 150 ms, compared with 182 ms for
MariaDB. MariaDB won the average and p(95), while MySQL handled the slowest one percent of
requests more consistently in this run.
...
| Metric | MariaDB 10.3.39 | MySQL 8.0.42 | MySQL 8.0.42 (`performance_schema=OFF`) |
|---|---:|---:|---:|
| Average / p(95) | **125.12 / 130.26 ms** | 130.25 / 136.35 ms | 130.81 / 136.82 ms |
| Database CPU average / peak | **0.2% / 1.0%** | 6.6% / 9.0% | 8.3% / 10.0% |
| Database RSS | **141 MB** | 411 MB | 162 MB |
...
> 10.3 was slightly faster on average and used less CPU than MySQL 8.0.

It does not prove that MariaDB is always faster than MySQL. It also does not show how either
engine would behave with large joins, analytical queries, writes under contention, replication,
different indexes, or a much larger dataset.
...
The simple-query prediction held for mpazari.com. MariaDB was approximately 5% faster on average,
7% faster at p(95), and used much less CPU in the smaller comparison. MySQL had the better p(99)
in the main run, and its apparent memory disadvantage largely disappeared when
`performance_schema` was disabled.
...
and this real application showed that pattern. But the advantage was small, workload-specific,
and not enough to make MySQL an unreasonable choice.**
